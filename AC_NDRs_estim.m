...
% This function computes the asymptotic Algorithmic Complexity per estimation for the Mixture of Delta Rules with N>1 units.
% NSP=1 --> Neural-Spiking P system implementation of the algorithm.
% NSP=0 --> Each operation counts 1.

function[f]=AC_NDRs_estim(N,NSP)
...
if(NSP)
    ...
    f = AC_NDRs_pred(N,NSP) -(S+A+2*P) - 3*Mr  - Mm;
    ...
    f = AC_NDRs_pred(N,NSP) -(2*S+2*P) - 3*Mr  - Mm;
