...
% This function computes the asymptotic Algorithmic Complexity per estimation for the Memoryless model.
% NSP=1 --> Neural-Spiking P system implementation of the algorithm.
% NSP=0 --> Each operation counts 1.


function[f]=AC_ML_estim(NSP)
...
if(NSP)
    ...
    f = AC_ML_pred(NSP) -(S+A+2*P) - 3*Mr - Mm;
    ...
    f = AC_ML_pred(NSP) -(2*S+2*P) - 3*Mr - Mm;
